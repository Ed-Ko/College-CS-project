...
for i=1:length(NumSta)
    for j=1:NumIter
        ...
        for a=1:NumSta(i)
            backoff(1,a) = randi(CW(a));                     % Initial backoff counter
        end
        ...
        for t=1:SimuTime
            ...
            [InterPktTime, StaPktQ, StaPktCnt] = generate_pkt_CSMA(InterPktTime, nn, PktLen, SlotTime, StaPktQ, StaPktCnt,lambda);
            ...
            if (~isBusy)
                ...
                if (min(backoff(uid)) == 0)
                    ...
                    if length(winner) == 1
                        backoff(winner(1)) = CWmin;
                    else
                        for a=1:length(winner)
                            if CW(winner(a)).*2 <= 1024
                                backoff(winner(a)) = randi(CW(winner(a)).*2);
                            else
                                backoff(winner(a)) = randi(1024);
                            end
                        end
                    end
...
for i=1:length(NumSta)
    for j=1:NumIter
        ...
        for t=1:SimuTime
            sprintf("=========== timeslot: %d ============", t)
            % schedule packet arrival events
            [InterPktTime, StaPktQ, StaPktCnt] = generate_pkt_baseline(InterPktTime, nn, PktLen, SlotTime, StaPktQ, StaPktCnt,lambda);
...
legend('baseline');
grid;
saveas(gcf,'rate_baseline.jpg','jpg')

function [InterPktTime, StaPktQ, StaPktCnt] = generate_pkt_CSMA(InterPktTime, nn, PktLen, SlotTime, StaPktQ, StaPktCnt,lambda)
...
for u=1:length(InterPktTime)
    if(floor(InterPktTime(u)) == 0)
        ...
        proba_arr = rand(1,nn);
        for a=1:nn
            InterPktTime(a) = -1*(log(1-proba_arr(a))/lambda(a)); % TODO: update the interarrival time her
        end
...
function [InterPktTime, StaPktQ, StaPktCnt] = generate_pkt_baseline(InterPktTime, nn, PktLen, SlotTime, StaPktQ, StaPktCnt,lambda)
InterPktTime = InterPktTime - 1;
for u=1:length(InterPktTime)
    if(floor(InterPktTime(u)) == 0)
        StaPktCnt(u) = StaPktCnt(u) + 1;
        StaPktQ(u,StaPktCnt(u)) = PktLen;
        proba_arr = rand(1,nn);
        InterPktTime(u)=1/lambda(u)/SlotTime; % TODO: update the interarrival time her
