...
StaWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::StaWifiMac")
    ...
    .AddAttribute ("SNR_Threshold", "threshold of the signal quality (dB)",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&StaWifiMac::m_snrThreshold),
...
void
StaWifiMac::SendDisassociationMsg(void)
{
  // [TODO]: Send disassociation message to the AP 
  WifiMacHeader hdr;
      ...
      if (goodBeacon && m_state == ASSOCIATED)
        {
          ...
          // [TODO]: 
          // 1. Extract SNR info. from beacon packet
	  SnrTag snrTag;
	  bool removed = copy->RemovePacketTag(snrTag);
	  NS_ASSERT(removed);

          // 2. Do linear, log scale conversion
	  double snr_found = 10 * log10(snrTag.Get());


          // 3. Compare current SNR with the threshold
	  bool bigger = false;
	  if(snr_found > m_snrThreshold)
	  	  bigger = true;

          // 4. Corrupt the connection if the connection quality is too bad
	  
	  if(bigger == false){
	  	SendDisassociationMsg();
	        SetState(UNASSOCIATED);
		//TryToEnsureAssociated();
		m_linkDown();
		SendProbeRequest();
...
StaWifiMac::UpdateCandidateApList (ApInfo newApInfo)
{
  ...
  // [TODO]: Do not insert to the candiate list if new AP's SNR < threshold
  // Note: Remember to do linear scale, log scale conversion   

  // Insert before the entry with lower SNR
  if(10 * log10(newApInfo.m_snr) > m_snrThreshold){
  	for (std::vector<ApInfo>::iterator i = m_candidateAps.begin(); i != m_candidateAps.end(); ++i)
        {
