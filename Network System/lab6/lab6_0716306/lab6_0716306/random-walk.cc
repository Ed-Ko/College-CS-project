...
void Movement (Ptr<Node> node, double stepsSize, double stepsTime, FILE *fp, std::vector<Vector> boundary);
...
int main (int argc, char *argv[])
{
  ...
  Simulator::Schedule (Seconds(stepsTime), &Movement, wirelessNodes.Get(0), stepsSize, stepsTime, fp, boundary);
...
void Movement (Ptr<Node> node, double stepsSize, double stepsTime, FILE *fp, std::vector<Vector> boundary)
{
  ...
  MOVE move_now = GetSingleMovement(fp); 
  double add_x = move_now.speed * cos(move_now.rad);
  double add_y = move_now.speed * sin(move_now.rad);

  
  Ptr<MobilityModel> mob = node->GetObject<MobilityModel>();
  Vector cur_pos = mob->GetPosition();

  if(cur_pos.x + add_x < boundary[0].x && cur_pos.x + add_x > boundary[1].x && cur_pos.y + add_y < boundary[0].y && cur_pos.y + add_y > boundary[1].y){
  	cur_pos.x += add_x;
	cur_pos.y += add_y;
	mob->SetPosition(cur_pos);
	//std::cout << "current position : " << "(" << mob->GetPosition().x << "," << mob->GetPosition().y << "," << mob->GetPosition().z << ")" << std::endl;

  }
  else{
  	PrintIllegalMsg(move_now.index);
  }

  CheckLinkState(node); 
  if(!feof(fp))
  	Simulator::Schedule (Seconds(stepsTime), &Movement, node, stepsSize, stepsTime, fp, boundary);
	//Movement(node, stepsSize, stepsTime, fp, boundary);
  else
	return;
